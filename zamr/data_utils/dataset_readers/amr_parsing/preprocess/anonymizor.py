...
from zamr.data_utils.dataset_readers.amr_parsing.amr import AMR
...
class Anonymizor:
    ...
    def _leave_as_is(self,
        ...
        if anonym_type == 'date-entity':
            ...
            if (amr.lemmas[index].isdigit() and len(amr.lemmas[index]) < 4 and (
                ...
                return True
            if amr.lemmas[index].isalpha() and (
                    prev_token_is(index, 1, amr, self._INVP[3]) or next_token_is(index, 1, amr, self._INVS[3])):
                return True

        if anonym_type == 'quantity':
            if len(amr.lemmas[index]) == 1 and prev_token_is(index, 2, amr, self._INVP[4]) and next_token_is(index, 1,
                                                                                                             amr,
                                                                                                             self._INVP[
                                                                                                                 4]):
                return True
    ...
    @classmethod
    def from_json(cls, file_path: str) -> 'Anonymizor':
