...
class Cleaner:
    ...
    def __call__(self, amr):
        self.correct_errors(amr)
        self.normalize_tokens(amr)
        # Named entity
        self.join_model_name(amr)
        self.split_entity_with_slash(amr)
        self.split_entity_with_non(amr)
        self.split_entity_prefix(amr, 'anti')
        self.split_entity_prefix(amr, 'ex')
        self.split_entity_prefix(amr, 'cross')
        self.split_entity_prefix(amr, 'pro')
        self.replace_NT_dollar_abbr(amr)
        # Date
        self.join_time_description(amr)
        self.split_date_duration(amr)
        self.split_numerical_date(amr)
        self.split_year_month(amr)
        self.split_era(amr)
        self.split_911(amr)
        self.split_ratio(amr)
        self.split_unit_with_number(amr)
        self.split_number_with_dash_prefix(amr)

    def correct_errors(self, amr):
        while True:
            index = None
            for i, token in enumerate(amr.tokens):
                ...
                if ((amr.id.startswith('PROXY_AFP_ENG_20071030_0313.5') or
                    ...
                    ner = []
                    break
                if amr.id.startswith('PROXY_AFP_ENG_20050603_0056.11') and token == 'first' and amr.tokens[
                    i - 1] == "'s":
                    index = i
                    ...
                    ner = ['ORDINAL', 'O']
                    break
                if amr.id.startswith('PROXY_AFP_ENG_20070327_0002.14') and token == 'first' and amr.tokens[
                    i + 1] == 'time':
                    index = i
                    ...
                    ner = ['O', 'ORDINAL']
                    break
                if amr.id.startswith('DF-200-192400-625_7046.5') and token == 'my' and amr.tokens[
                    i + 1] == 'counsellers':
                    index = i
                    tokens = ['my', '2']
                    pos = ['PRP$', 'CD']
                ...
                if amr.id.startswith('PROXY_LTW_ENG_20081115_0076.19') and token == 'a' and amr.tokens[i + 1] == 'year':
                    ...
                    ner = ['NUMBER', 'DURATION']
                    break
                if amr.id.startswith('PROXY_XIN_ENG_20020905_0122.11') and token == 'separate' and amr.tokens[
                    i + 1] == 'bomb':
        ...
        while True:
            ...
            for i, lemma in enumerate(amr.lemmas):
                ...
                if lemma_lower == 'midnight':
                    ...
                    ner = ['TIME']
                    break
                if token_lower in self.DECADE_MAP:
                    span = [i]
                    tokens = [str(self.DECADE_MAP[token_lower])]
                    pos = ['CD']
                    ner = ['TIME']
                    break
                if lemma_lower in self.ORDINAL_MAP:
                    span = [i]
                    tokens = [str(self.ORDINAL_MAP[lemma_lower])]
                    pos = ['CD']
                    ner = ['ORDINAL']
                    break
    ...
    def join_time_description(self, amr):
    ...
    def split_entity_with_slash(self, amr):
    ...
    def split_entity_with_non(self, amr):
    ...
    def split_entity_prefix(self, amr, prefix):
    ...
    def split_unit_with_number(self, amr):
    ...
    def split_number_with_dash_prefix(self, amr):
    ...
    def split_date_duration(self, amr):
        # 201005-201006
        while True:
            index = None
            x = None
    ...
    def split_numerical_date(self, amr):
    ...
    def replace_NT_dollar_abbr(self, amr):
