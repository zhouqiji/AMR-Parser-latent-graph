...
from zamr.data_utils.vocabulary import DEFAULT_PADDING_TOKEN, DEFAULT_OOV_TOKEN
from zamr.data_utils.dataset_readers.amr_parsing.graph_repair import GraphRepair
from zamr.utils.string import find_similar_token, is_abstract_token, is_english_punct
...
class AMR:
    ...
    def get_named_entity_span(self, index):
        ...
        while prev > 0 and self.ner_tags[prev] == tag:
            ...
            prev -= 1
        _next = index + 1
        while _next < len(self.ner_tags) and self.ner_tags[_next] == tag:
            span.append(_next)
            _next += 1
        return span
