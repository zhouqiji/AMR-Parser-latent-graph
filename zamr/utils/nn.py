...
def sort_batch_by_length(tensor: torch.Tensor, sequence_lengths: torch.Tensor):
    ...
    index_range = torch.arange(0, sequence_lengths.size(0), dtype=sequence_lengths.dtype,
...
def get_dropout_mask(dropout_probability: float, tensor_for_masking: torch.Tensor):
    ...
    binary_mask = torch.rand_like(tensor_for_masking) > dropout_probability
...
def get_text_field_mask(text_field_tensors: Dict[str, torch.Tensor],
    ...
    if smallest_dim == 2:
        ...
        return (character_tensor > 0).long().sum(dim=-1) > 0
    else:
