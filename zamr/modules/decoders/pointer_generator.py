...
class PointerGenerator(torch.nn.Module):
    ...
    def forward(self, hiddens, source_attentions, source_attention_maps,
        ...
        # [batch_size, num_target_nodes, num_target_nodes]
        # make sure for target_node_i, its attention to target_node_j >= target_node_i
        # should be zero.
    ...
    def compute_loss(self, probs, predictions, generate_targets,
        ...
        source_copy_target_probs = source_copy_target_probs.mul(non_target_copy_mask.float()).mul(
            source_copy_mask.float())
        ...
        num_correct_source_point = predictions.ge(self.vocab_size).mul(
            predictions.lt(self.vocab_size + source_dynamic_vocab_size)). \
