...
from zamr.utils.checks import check_for_gpu, ConfigurationError
from zamr.utils import lazy_groups_of
from zamr.predictors.predictor import Predictor, JsonDict
from zamr.predictors import ParserPredictor
...
class Predict:
    ...
    def add_subparser(name, parser):
        ...
        subparser.add_argument('--archive-file', required=True, type=str,
        ...
        subparser.add_argument('--visual', action='store_true', help='return both amr and dep/latent graphs.')
...
def _get_predictor(args):
    from zamr.utils.archival import load_archive
...
class _PredictManager:
    ...
    def __init__(self,
        ...
        if type(predictor) in (ParserPredictor,):
    ...
    def _maybe_print_to_console_and_file(self,
        ...
        prediction, dep_graph, latent_graph = prediction
        ...
        if visual:
            global graphs
            dep_graph, latent_graph = torch.tensor(dep_graph), torch.tensor(latent_graph)
            graphs.append((dep_graph, latent_graph))
    ...
    def visual_run(self):
        ...
        if has_reader:
            for batch in lazy_groups_of(self._get_instance_data(), self._batch_size):
                for model_input_instance, result in zip(batch, self._predict_instances(batch)):
                    self._maybe_print_to_console_and_file(result, str(model_input_instance), visual=True)
