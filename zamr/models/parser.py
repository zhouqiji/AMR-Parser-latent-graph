...
from zamr.models.model import Model
from zamr.utils.logging import init_logger

# Graph encoder
from zamr.modules.graph_encoders import GraphEncoder

from zamr.modules.seq2seq_encoders import Seq2SeqBertEncoder
from zamr.modules.token_embedders.embedding import Embedding
from zamr.modules.seq2vec_encoders.cnn_encoder import CnnEncoder
from zamr.modules.seq2seq_encoders.pytorch_seq2seq_wrapper import PytorchSeq2SeqWrapper
from zamr.modules.stacked_bilstm import StackedBidirectionalLstm
from zamr.modules.stacked_lstm import StackedLstm
from zamr.modules.decoders.rnn_decoder import InputFeedRNNDecoder
from zamr.modules.attention_layers.global_attention import GlobalAttention
from zamr.modules.attention import DotProductAttention
from zamr.modules.attention import MLPAttention
from zamr.modules.attention import BiaffineAttention
from zamr.modules.input_variational_dropout import InputVariationalDropout
from zamr.modules.decoders.pointer_generator import PointerGenerator
from zamr.modules.decoders.deep_biaffine_graph_decoder import DeepBiaffineGraphDecoder
from zamr.utils.nn import get_text_field_mask
from zamr.utils.string import START_SYMBOL, END_SYMBOL, find_similar_token, is_abstract_token
from zamr.data_utils.vocabulary import DEFAULT_OOV_TOKEN, DEFAULT_PADDING_TOKEN
from zamr.data_utils.tokenizers.character_tokenizer import CharacterTokenizer

# The following imports are added for mimick testing.
from zamr.data_utils.dataset_builder import load_dataset_reader
from zamr.predictors.predictor import Predictor
from zamr.cmds.predict import _PredictManager
...
class PARSER(Model):
    def __init__(self,
        ...
        self.decoder_token_indexers = None
        self.character_tokenizer = None
    ...
    def mimick_test(self):
        ...
        predictor = Predictor.by_name('PARSER')(self, dataset_reader)
    ...
    @staticmethod
    def print_batch_details(batch, batch_idx):
    ...
    def forward(self, batch, for_training=False):
        ...
        encoder_outputs = self.encode(
            ...
            encoder_inputs['char'],
            encoder_inputs['mask'],
            encoder_inputs['dep_heads'],
        )
        dep_graph = encoder_outputs['dependent_graph']
        latent_graph = encoder_outputs['latent_graph']
    ...
    def encode(self, bert_tokens, token_subword_index, tokens, pos_tags, must_copy_tags, chars, mask, dep_heads):
        ...
        # Graph encoders
        graph_encoder_outputs, latent_graph, dependent_graph = self.graph_encoder(encoder_inputs,
                                                                                  mask,
                                                                                  dep_heads)
        encoder_outputs = self.encoder(graph_encoder_outputs, mask)
        ...
        return dict(
            ...
            latent_graph=latent_graph,
            dependent_graph=dependent_graph,
    ...
    def decode(self, input_dict):
        ...
        return dict(
            ...
            dep_graph=input_dict['dep_graph'],
            latent_graph=input_dict['latent_graph']
        )
    ...
    def beam_search_with_pointer_generator(
        ...
        def get_decoder_input(tokens, pos_tags, corefs):
            token_embeddings = self.decoder_token_embedding(tokens.long())
            pos_tag_embeddings = self.decoder_pos_embedding(pos_tags.long())
            coref_embeddings = self.decoder_coref_embedding(corefs.long())
        ...
        return_dict["scores"] = torch.div(return_dict["scores"],
                                          return_dict["decoder_mask"].sum(1, keepdim=True).type_as(
                                              return_dict["scores"]))
    ...
    def from_params(cls, vocab, params):
        # Build the model
        logger.info('Building the Parser...')
        ...
        # Graph Encoder
        graph_type = params['graph_encoder']['graph_type']

        params['graph_encoder']['input_size'] = encoder_input_size
        graph_encoder = GraphEncoder.from_params(params['graph_encoder'])

        params['encoder']['input_size'] = params['graph_encoder']['gcn_hidden'] * 2
        ...
        generator = PointerGenerator(
            ...
            vocab_pad_idx=0,
            graph_encoder_type=params['graph_encoder']['graph_type']
        )
