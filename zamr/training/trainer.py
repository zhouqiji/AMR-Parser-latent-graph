...
from zamr.training.tensorboard_utils import TensorboardWriter
from zamr.utils.environment import device_mapping, peak_memory_mb, gpu_memory_mb, move_to_device
from zamr.utils.checks import ConfigurationError
from zamr.utils.tqdm import Tqdm
from zamr.utils.time import time_to_str
...
class Trainer:
    ...
    def _batch_loss(self, batch, for_training: bool) -> (torch.Tensor, torch.Tensor):
        """
    ...
    def _train_epoch(self, epoch):
        ...
        for batch in train_generator_tqdm:
            ...
            clip_grad_norm_([p for p in self._model.parameters() if p.requires_grad], self._max_grad_norm)
    ...
    def train(self):
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            self._scheduler.step(dev_metrics['F1'])
            # Save status.
            ...
            self._tensorboard.add_dev_scalar('learning_rate', self._optimizer.param_groups[0]['lr'], epoch)
    ...
    @classmethod
    def from_params(cls, model, train_data, dev_data, train_iterator, dev_iterator, params, graph_type='rectified'):
        ...
        optimizer = optim.Adam(parameters, lr=lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=3e-9)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=5, factor=0.9, min_lr=5e-5,
                                                         verbose=True)
